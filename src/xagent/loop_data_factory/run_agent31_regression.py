...
def run_regression(
    *,
    ...
    resolved_llm_judge_config = llm_judge_config or LLMJudgeConfig.from_env()
    ...
    for case_ref in cases:
        ...
        rule_judge = judge_agent_output(case, raw_output)
        llm_judge = judge_agent_output_with_llm(
            case,
            raw_output,
            rule_judge,
            config=resolved_llm_judge_config,
        )
        judge = combine_rule_and_llm_judges(rule_judge, llm_judge)
        result = {
            "case_id": case["case_id"],
            "loop_type": case["loop_type"],
            "dataset_version": manifest.get("dataset_version"),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="Enable optional LLM semantic judge. Defaults to env XAGENT_LOOP_LLM_JUDGE_ENABLED.",
    )
    parser.add_argument(
        "--llm-judge-api-base",
        default=None,
        help="OpenAI-compatible base URL for LLM judge, for example https://api.openai.com/v1.",
    )
    parser.add_argument(
        "--llm-judge-api-key",
        default=None,
        help="API key for LLM judge. Prefer env XAGENT_LOOP_LLM_JUDGE_API_KEY.",
    )
    parser.add_argument(
        "--llm-judge-model",
        default=None,
    ...
    llm_judge_config = LLMJudgeConfig.from_env(enabled=args.llm_judge).with_overrides(
...
def _build_summary(
    *,
    ...
    return {
        ...
        "stopped_early": stopped_early,
        "llm_judge": {
            "enabled": any(bool(item.get("enabled")) for item in llm_judge_results),
            "evaluated": sum(
                1 for item in llm_judge_results if item.get("status") == "evaluated"
            ),
            "errors": sum(
                1 for item in llm_judge_results if item.get("status") == "error"
            ),
            "estimated_cost_usd": round(sum(llm_costs), 8) if llm_costs else None,
        },
