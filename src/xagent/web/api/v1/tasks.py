...
async def create_chat_task(
    ...
    perspective:
      ...
      2. Persists a new :class:`Task` row owned by the agent's user,
         with ``source='sdk'``, ``is_visible=True``, and ``input`` set
    ...
    # from web/widget. SDK tasks are visible so deployments that run
    # regression or external API traffic can inspect those conversations
    # from the same frontend task list as Web UI runs. ``input`` records
    # this turn's user message so GET endpoint can return it without
    # going through task_chat_messages.
    task = Task(
